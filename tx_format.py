...
class tx_object:
    def __init__(self, origin, owner,value, owner_pubkey,upper_limit, previous_hash=None, current_hash=None):
        ...
        temp=str(self.previous_hash)+str(self.origin)+str(self.owner)+str(self.owner_pubkey)+str(self.value)+str(self.upper_limit)+str(self.current_hash)
        
        temp=SHA512.new(bytes(temp))
        self.current_hash=temp.digest()
